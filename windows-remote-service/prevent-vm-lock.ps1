...
param(
    [string]$Mode = "interactive",  # interactive, service, tscon-only
...
function Invoke-TsconPrevention {
    try {
        ...
        # If no disconnected sessions, check for active RDP sessions that might be at risk
        $activeSessions = $sessions | Where-Object { 
            $_.Username -eq $currentUser -and $_.State -eq "Active" -and $_.SessionName -like "rdp-tcp*"
        }
        
        if ($activeSessions.Count -gt 0) {
            Write-LogMessage "Found $($activeSessions.Count) active RDP session(s) - monitoring for disconnection"
        }
...
function Set-PowerManagementSettings {
    ...
    try {
        ...
        if ($highPerf) {
            ...
            Write-LogMessage "Set power plan to High Performance: $highPerf"
        }
...
function Start-LockPreventionService {
    ...
    while ($true) {
        try {
            ...
            if ($Verbose) {
                Write-LogMessage "Status check - Sessions: $($status.CurrentSessions.Count), Elevated: $($status.IsElevated)"
            }
...
switch ($Mode.ToLower()) {
    "interactive" {
        Write-Host "🔒 Windows VM Lock Prevention Utility"
        Write-Host "=================================="
    ...
    default {
        ...
        Write-Host "Valid modes: interactive, service, tscon-only, status"
        exit 1
    }
} 