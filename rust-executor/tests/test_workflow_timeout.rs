...
async fn test_unreachable_mcp_server_times_out() {
    ...
    match result {
        Ok(Err(e)) => {
            ...
            assert!(elapsed < Duration::from_secs(65), "Should fail within 65s (30s connect timeout + overhead)");
        },
        Err(_) => {
            panic!("❌ Hit outer timeout - connection timeout not working! Took {:?}", elapsed);
        },
        Ok(Ok(_)) => {
...
async fn test_hanging_mcp_request_times_out() {
    ...
    assert!(result.is_ok(), "Should fail with connection error, not outer timeout");
    assert!(elapsed < Duration::from_secs(32), "Should timeout within 32s");
}
...
async fn test_workflow_execution_overall_timeout() {
    ...
    assert!(elapsed < Duration::from_secs(605), "Should timeout at ~600s");
    assert!(elapsed > Duration::from_secs(595), "Should timeout after 10 minutes");
}
...
async fn test_http_request_timeout_configuration() {
    ...
    assert!(elapsed < Duration::from_secs(35), "Should timeout within connect timeout");
}
