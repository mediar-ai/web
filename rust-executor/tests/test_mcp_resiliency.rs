...
async fn http_initialize_503_backoff_then_success_and_tool_call() {
    ...
    Mock::given(method("POST")).and(path("/mcp")).and(body_string_contains("notifications/initialized")).respond_with(notify_ok)
        .mount(&server)
        .await;
    ...
    let res = client
        .execute_tool_with_timeout("test_tool".to_string(), Some(serde_json::Map::new()), Some(5_000))
        .await
...
async fn tool_call_401_triggers_reinit_and_retry() {
    ...
    assert_eq!(res.get("status").and_then(|v| v.as_str()), Some("recovered"));
}
