...
import { deallocateVm, isAzureConfigured } from '@/lib/azure';
...
export async function POST(
  ...
  try {
    ...
    // Deallocate the VM
    const result = await deallocateVm(machine.azure_resource_id);
    ...
    if (!result.success) {
      return NextResponse.json(
        {
          error: result.error || 'Deallocate failed',
    ...
    const errorMessage =
      error instanceof Error ? error.message : 'Deallocate failed';
