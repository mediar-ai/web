...
interface PostHogPerson {
  ...
  properties: {
    email?: string;
    $initial_person_info?: { $initial_distinct_id?: string };
  };
  created_at: string;
}
...
async function getInactiveUsers(daysAgo: number = 2): Promise<{ email: string; userId: string; signupDate: string }[]> {
  ...
  // Get users who have user_created event but NOT desktop_app_started
  // Using HogQL query
  const query = `
    SELECT
...
export async function POST(request: NextRequest) {
  ...
  try {
    // Get users who signed up 2+ days ago but never opened app
