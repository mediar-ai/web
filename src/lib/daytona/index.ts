...
app.use('*', cors({
  ...
  allowHeaders: ['Content-Type', 'Authorization', 'x-daytona-preview-token', 'x-mediar-auth'],
}));
...
app.post('/api/ai', async (c) => {
  // Authenticate - use x-mediar-auth to avoid Daytona proxy intercepting Authorization header
  const authHeader = c.req.header('x-mediar-auth');
  ...
  if (existing) {
    try {
      ...
      if (sandbox.state === 'started') {
        ...
        await supabase
          .from('user_sandboxes')
          .update({ last_accessed_at: info.lastAccessedAt })
          .eq('user_id', userId);
      ...
      console.log(`Starting stopped sandbox for user: ${userId}`);
      await sandbox.start();
      await waitForSandboxReady(sandbox);

      const preview = await sandbox.getPreviewLink(3000);
      console.log(`Sandbox started, got new preview URL: ${preview.url}, token: ${preview.token ? preview.token.substring(0, 8) + '...' : 'MISSING'}`);
      ...
      // Update database with new preview URL and token (they can change on restart)
      await supabase
        .from('user_sandboxes')
        .update({
      ...
      console.error('Failed to get existing sandbox, creating new one:', error);
...
export async function stopSandbox(userId: string): Promise<void> {
  ...
  try {
    const sandbox = await daytona.get(cached.sandboxId);
    await sandbox.stop();

    cached.state = 'stopped';
    sandboxCache.set(userId, cached);
  } catch (error) {
