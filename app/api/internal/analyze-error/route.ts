...
1. MCP connection timeouts (http://13.77.110.245:8080/mcp or similar endpoints)
...
function generateBasicAnalysis(data: ErrorAnalysisRequest): string {
  ...
  if (error?.message?.includes('ConnectTimeout') || error?.message?.includes('http://13.77.110.245:8080/mcp')) {
    analysis += '**Root Cause:** MCP connection timeout - The automation server is not responding.\n\n';
    analysis += '**Solution:**\n';
    analysis += '- Check if the MCP server at 13.77.110.245 is running\n';
