-- Migration: Fix workflow statistics view to use correct column name
-- Created: 2026-10-27
-- Description: Fixes the workflow_statistics_summary view to use version_number instead of workflow_version_number

-- Drop the existing view to avoid column order issues
DROP VIEW IF EXISTS workflow_statistics_summary;

-- Recreate the view with the correct column name
CREATE VIEW workflow_statistics_summary AS
SELECT
    w.id,
    w.name,
    w.description,
...
LEFT JOIN LATERAL (
    ...
    AND we.version_number = w.version
) cv ON true;

-- Add description column that was missing
COMMENT ON VIEW workflow_statistics_summary IS 'Aggregates workflow statistics including version-specific metrics for the current active version';

-- Grant permissions
GRANT SELECT ON workflow_statistics_summary TO authenticated;
GRANT SELECT ON workflow_statistics_summary TO anon;
