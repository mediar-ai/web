...
CREATE FUNCTION public.calculate_and_update_labeling_stats_v2()
RETURNS TRIGGER AS $$
DECLARE
    v_session_id TEXT;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_analysis_label_update_v2') THEN
        CREATE TRIGGER on_analysis_label_update_v2
        AFTER UPDATE OF label_status ON public.low_level_workflow_analyses
        FOR EACH ROW
        EXECUTE FUNCTION public.calculate_and_update_labeling_stats_v2();
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_new_dataset_entry_v2') THEN
        CREATE TRIGGER on_new_dataset_entry_v2
