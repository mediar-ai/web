...
__export(azure_exports, {
  AzureResourceParseError: () => AzureResourceParseError,
  POWER_STATE_MAP: () => POWER_STATE_MAP,
  VNC_GATEWAY_URL: () => VNC_GATEWAY_URL,
  buildAzureResourceId: () => buildAzureResourceId,
  buildAzureVmResourceId: () => buildAzureVmResourceId,
  buildImage: () => buildImage,
  checkImageBuilderPrerequisites: () => checkImageBuilderPrerequisites,
  createImageTemplate: () => createImageTemplate,
  deallocateVm: () => deallocateVm,
  deleteImageTemplate: () => deleteImageTemplate,
  extractHostFromEndpoint: () => extractHostFromEndpoint,
  getAzureCredential: () => getAzureCredential,
  getComputeClient: () => getComputeClient,
  getComputeClientForSubscription: () => getComputeClientForSubscription,
  getLatestGalleryImageVersion: () => getLatestGalleryImageVersion,
  getNetworkClient: () => getNetworkClient,
  getSubscriptionId: () => getSubscriptionId,
  getVmPublicIp: () => getVmPublicIp,
  getVmState: () => getVmState,
  isAzureConfigured: () => isAzureConfigured,
  isValidAzureResourceId: () => isValidAzureResourceId,
  isValidAzureVmResourceId: () => isValidAzureVmResourceId,
  isVmReady: () => isVmReady,
  listImageTemplates: () => listImageTemplates,
  parseAzureResourceId: () => parseAzureResourceId,
  parseAzureVmResourceId: () => parseAzureVmResourceId,
...
function getComputeClientForSubscription(subscriptionId) {
  const cred = getAzureCredential();
  return new import_arm_compute.ComputeManagementClient(cred, subscriptionId);
}
...
function getImageBuilderClient() {
  if (!imageBuilderClient) {
    ...
    imageBuilderClient = new import_arm_imagebuilder.ImageBuilderClient(credential2, subscriptionId);
  }
...
async function getLatestGalleryImageVersion() {
  const subscriptionId = getSubscriptionId();
  const credential2 = getAzureCredential();
  const computeClient2 = new import_arm_compute2.ComputeManagementClient(credential2, subscriptionId);
  try {
    const versions = [];
