...
(SKIP_TESTS ? describe.skip : describe)(
  '/api/stream-proxy - Real Vertex AI Integration',
  () => {
    const mockAuth = `Bearer ${process.env.AI_API_PASSWORD || 'your-secret-password-here'}`;
    ...
    it('should handle click_element tool with const and anyOf', async () => {
      const clickTool = mcpToolSchemas.find(
        (t: any) => t.name === 'click_element'
      );
    ...
    it('should handle execute_sequence tool with $ref and definitions', async () => {
      const execTool = mcpToolSchemas.find(
        (t: any) => t.name === 'execute_sequence'
      );
        ...
        'http://localhost:3000/api/stream-proxy',
        {
          ...
          body: JSON.stringify({
            messages: [
              {
                role: 'user',
                content: 'List all available tools and their purposes',
              },
            ],
            tools: mcpToolSchemas,
            temperature: 0.5,
