...
program
  .option(
    "-m, --with-source-maps [type]",
    "output source maps when not generating a bundled app (type may be empty for external source maps, inline for inline source maps, or both) "
  )
...
function makeLibFiles(sourceMaps) {
  ...
  const handleDir = (vendorRewrite, inPathBase, filename) =>
      ...
      return Promise.resolve()
        ...
        .then(() => {
          ...
          if (vendorRewrite) {
            opts.plugins.push([
              "import-redirect",
              { root: paths.libDirBase, redirect: { "vendor/(.+)": "./vendor/$1" } },
            ]);
          }

          return babelTransformFile(filename, opts).then(res => {
            ...
            return writeFile(outPath, code).then(() => {
