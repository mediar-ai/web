...
const UI = {
  ...
  addClipboardHandlers() {
    ...
    // Track last synced clipboard to avoid duplicates
    UI._lastSyncedClipboard = "";

    // Auto-sync clipboard: when VNC container gets focus, read host clipboard and send to VM
    document.getElementById("noVNC_container").addEventListener("focus", UI.syncClipboardToVM, true);
    document.getElementById("noVNC_container").addEventListener("click", UI.syncClipboardToVM);

    // Intercept Ctrl+V to ensure clipboard is synced before paste
    document.addEventListener("keydown", UI.handleCtrlV, true);

    // Manual clipboard buttons
    document.getElementById("noVNC_paste_from_host").addEventListener("click", UI.pasteFromHost);
    document.getElementById("noVNC_type_to_vm").addEventListener("click", UI.typeToVM);
  },

  // Button: Read host clipboard into textarea
  async pasteFromHost() {
    console.log("[noVNC Clipboard] Paste from Host clicked");
    try {
      if (navigator.clipboard && navigator.clipboard.readText) {
        const text = await navigator.clipboard.readText();
        console.log("[noVNC Clipboard] Got from host:", text.substr(0, 50) + (text.length > 50 ? "..." : ""));
        document.getElementById("noVNC_clipboard_text").value = text;
        // Also sync to VM clipboard
        if (UI.rfb && UI.connected) {
          UI.rfb.clipboardPasteFrom(text);
          console.log("[noVNC Clipboard] Sent to VM clipboard");
        }
      } else {
        console.error("[noVNC Clipboard] navigator.clipboard.readText not available");
        alert("Clipboard API not available. Make sure you're on HTTPS or localhost.");
      }
    } catch (err) {
      console.error("[noVNC Clipboard] Failed to read clipboard:", err);
      alert("Failed to read clipboard: " + err.message + "\n\nMake sure you've granted clipboard permission.");
    }
  },

  // Button: Type textarea content to VM
  typeToVM() {
    const text = document.getElementById("noVNC_clipboard_text").value;
    if (!text) {
      alert("Textarea is empty. Paste text first or use 'Paste from Host' button.");
      return;
    }
    if (!UI.rfb || !UI.connected) {
    ...
    console.log("[noVNC Clipboard] Typing to VM:", text.substr(0, 50) + (text.length > 50 ? "..." : ""));

    // Type each character
    for (const char of text) {
      const code = char.charCodeAt(0);
      if (char === "\n") {
        UI.rfb.sendKey(KeyTable.XK_Return, "Enter", true);
        UI.rfb.sendKey(KeyTable.XK_Return, "Enter", false);
      } else if (char === "\t") {
        UI.rfb.sendKey(KeyTable.XK_Tab, "Tab", true);
        UI.rfb.sendKey(KeyTable.XK_Tab, "Tab", false);
      } else if (code >= 32) {
        const keysym = keysyms.lookup(code);
        if (keysym) {
    ...
    console.log("[noVNC Clipboard] Finished typing", text.length, "characters");
    alert("Typed " + text.length + " characters to VM");
  },

  // Read host clipboard and send to VM (called on focus/click)
  async syncClipboardToVM() {
    if (!UI.rfb || !UI.connected) return;
    try {
      // Only works in secure context (HTTPS or localhost)
      if (navigator.clipboard && navigator.clipboard.readText) {
        const text = await navigator.clipboard.readText();
        if (text && text.length > 0 && text !== UI._lastSyncedClipboard) {
          console.log("[noVNC Clipboard] Syncing to VM:", text.substr(0, 50) + (text.length > 50 ? "..." : ""));
          UI._lastSyncedClipboard = text;
          document.getElementById("noVNC_clipboard_text").value = text;
          UI.rfb.clipboardPasteFrom(text);
        }
      }
      ...
      console.warn("[noVNC Clipboard] Read failed:", err.message);
    }
  },

  // Intercept Ctrl+V to type clipboard content directly (bypasses VM clipboard issues)
  async handleCtrlV(e) {
    if (!UI.rfb || !UI.connected) return;

    // Check if Ctrl+V (or Cmd+V on Mac)
    const isCtrlV = (e.ctrlKey || e.metaKey) && (e.key === "v" || e.key === "V");
    if (!isCtrlV) return;

    // Only handle if we're focused on VNC
    ...
    console.log("[noVNC Clipboard] Ctrl+V detected");

    // Prevent the default paste and VNC key event
    e.preventDefault();
    e.stopPropagation();
    ...
    try {
      ...
      if (navigator.clipboard && navigator.clipboard.readText) {
        ...
        if (text && text.length > 0) {
          console.log("[noVNC Clipboard] Typing directly:", text.substr(0, 50) + (text.length > 50 ? "..." : ""));
          UI._lastSyncedClipboard = text;
          document.getElementById("noVNC_clipboard_text").value = text;

          // Also try clipboard sync (might work on some VNC servers)
          UI.rfb.clipboardPasteFrom(text);

          // Type the text directly - this bypasses clipboard entirely
          for (const char of text) {
            const code = char.charCodeAt(0);
          ...
          console.log("[noVNC Clipboard] Clipboard is empty, sending regular Ctrl+V");
          UI.rfb.sendKey(KeyTable.XK_Control_L, "ControlLeft", true);
          UI.rfb.sendKey(keysyms.lookup("v".charCodeAt(0)), "KeyV", true);
          UI.rfb.sendKey(keysyms.lookup("v".charCodeAt(0)), "KeyV", false);
          UI.rfb.sendKey(KeyTable.XK_Control_L, "ControlLeft", false);
        }
      }
    } catch (err) {
      console.error("[noVNC Clipboard] Failed:", err);
      // Fallback: just send Ctrl+V
      UI.rfb.sendKey(KeyTable.XK_Control_L, "ControlLeft", true);
      UI.rfb.sendKey(keysyms.lookup("v".charCodeAt(0)), "KeyV", true);
      UI.rfb.sendKey(keysyms.lookup("v".charCodeAt(0)), "KeyV", false);
      UI.rfb.sendKey(KeyTable.XK_Control_L, "ControlLeft", false);
  ...
  clipboardReceive(e) {
    ...
    // Auto-sync to host clipboard
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(e.detail.text).catch(err => {
        Log.Debug("Failed to write to host clipboard: " + err);
      });
    }
