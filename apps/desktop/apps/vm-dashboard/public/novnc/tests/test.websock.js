...
describe("Websock", function () {
  ...
  describe("Queue methods", function () {
    ...
    describe("rQshift32()", function () {
      it("should pop four bytes from the receive queue and return a single number", function () {
        const befLen = sock.rQlen;
        const expected = (RQ_TEMPLATE[0] << 24) + (RQ_TEMPLATE[1] << 16) + (RQ_TEMPLATE[2] << 8) + RQ_TEMPLATE[3];
    ...
    describe("rQshiftStr", function () {
      it("should shift the given number of bytes off of the receive queue and return a string", function () {
        ...
        expect(shifted).to.equal(
          String.fromCharCode.apply(null, Array.prototype.slice.call(new Uint8Array(RQ_TEMPLATE.buffer, befRQi, 3)))
        );
  ...
  describe("WebSocket Receiving", function () {
    ...
    it("should compact the receive queue when a message handler empties it", function () {
      sock._eventHandlers.message = () => {
        sock.rQi = sock._rQlen;
      };
