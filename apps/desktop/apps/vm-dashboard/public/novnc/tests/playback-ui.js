...
function enableUI() {
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    frames[i] = { fromClient: frame[0] === "}", timestamp: time, data: u8 };
  }
...
class IterationPlayer {
  ...
  _finish() {
    ...
    const evt = new CustomEvent("finish", { detail: { duration: totalDuration, iterations: this._iterations } });
    this.onfinish(evt);
  }

  _iterationFinish(duration) {
    const evt = new CustomEvent("iterationfinish", { detail: { duration: duration, number: this._iteration } });
  ...
  _disconnected(clean, frame) {
    ...
    const evt = new CustomEvent("rfbdisconnected", {
      detail: { clean: clean, frame: frame, iteration: this._iteration },
    });
...
loadFile()
  .then(enableUI)
  .catch(e => message("Error loading recording: " + e));
