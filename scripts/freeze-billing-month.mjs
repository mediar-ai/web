 ...
 *
 * Usage:
 *   node scripts/freeze-billing-month.mjs <YYYY-MM> [--customer ExampleClient] [--base http://localhost:3000]
 *
 * What it does:
...
function parseArgs(argv) {
  const args = { month: null, customer: 'ExampleClient', base: 'http://localhost:3000' };
...
async function main() {
  ...
  if (!month || !/^\d{4}-\d{2}$/.test(month)) {
    console.error('Usage: node scripts/freeze-billing-month.mjs <YYYY-MM> [--customer ExampleClient] [--base http://localhost:3000]');
    process.exit(1);
  }
