...
async function testTypeScriptWorkflowSync() {
  ...
  // ==========================================================================
  const workflowPath = '../workflows/org-org_REDACTED/WORKFLOW_FOLDER';
  ...
  const { data: newWorkflow, error: createError } = await supabase
    .from('deployed_workflows')
    .insert({
      name: metadata.name || 'ExampleClient TypeScript Test',
      ...
      github_path: `org-org_REDACTED/${folderName}/src/terminator.ts`,
      github_ref: 'main',
      github_sync_status: 'synced',
      github_last_synced_at: new Date().toISOString(),
      version: '1.0.0',
      total_versions: 1,
      organization_id: 'org_REDACTED' // Default to your primary org
    })
    .select()
    .single();
