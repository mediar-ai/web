#!/usr/bin/env python3
"""
Backfill metadata table with extracted fields from existing low_level_events.
This approach avoids touching the massive main table.
"""
...
def backfill_batch(conn, offset: int, batch_size: int) -> int:
    """Process a batch of records and return the number processed."""
    cursor = conn.cursor()
    
    try:
        # Fetch batch of events that don't have metadata yet
        cursor.execute("""
        ...
        metadata_rows = []
        for event_id, payload_raw in records:
            try:
                # Parse JSON payload
        ...
        for _, event_type, app_name, has_ui_tree, screenshot_timestamp in metadata_rows:
        ...
        print(f"  ✅ Processed {len(metadata_rows)} records")
...
def get_total_unprocessed_count(conn) -> int:
    """Get total count of events without metadata."""
    cursor = conn.cursor()
    try:
        cursor.execute("""
            SELECT COUNT(*)
...
def main():
    ...
    print("🚀 Starting metadata table backfill")
    ...
    try:
        ...
        print(f"📊 Events without metadata: {total_unprocessed:,}")
        
        if total_unprocessed == 0:
            print("✅ All events already have metadata!")
            return
